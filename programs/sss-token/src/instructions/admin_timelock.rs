...
// ---------------------------------------------------------------------------
// set_oracle_params  (SSS-090: oracle staleness + confidence params)
// ---------------------------------------------------------------------------
...
///
/// - `max_age_secs`: Maximum seconds a Pyth price update can be old.
///   0 = use the hardcoded default (60 s).
/// - `max_conf_bps`: Maximum confidence-interval-to-price ratio in basis points.
///   E.g. 100 = 1%. 0 = confidence check disabled.
///
/// Authority-only call.  Recommended mainnet values: max_age_secs=60, max_conf_bps=100.
pub fn set_oracle_params_handler(
    ...
    msg!(
        "SSS-090: oracle params updated — max_age_secs={}, max_conf_bps={}",
        max_age_secs,
        max_conf_bps,
    );
    Ok(())
